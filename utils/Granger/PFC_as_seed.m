...
for cIndexPFC = 1:length(chsPFC)
    ...
    PATHs = dir(['dev\granger result\grangerres*PFC-', num2str(chsPFC(cIndexPFC)), '.mat']);
    ...
    mPrint(Fig, ['dev\Figures\From target AC-all to seed PFC-', num2str(chsPFC(cIndexPFC)), '.jpg'], "-djpeg", "-r200");
    ...
    addLines2Axes(struct("X", {0; t}, "Y", {[]; coi}, "color", "w", "width", 1.5));
    colorbar('position', [0.96, 0.1, 0.5 * 0.03, 0.8]);
    mPrint(Fig, ['dev\Figures\From seed PFC-', num2str(chsPFC(cIndexPFC)), ' to target AC-all.jpg'], "-djpeg", "-r200");
end