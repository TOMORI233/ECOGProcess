...
for mIndex = 1 : length(MATPATH)
    ...
    if ~exist(strcat(FIGPATH, protStr(mIndex), "_PopulationData.mat"), "file")
        ...
        save(strcat(FIGPATH, protStr(mIndex), "_PopulationData.mat"), "trialsECOG_S1_Merge", "trialsECOG_Merge", "trialAll");
    else
    ...
    [S1H{mIndex}, S1P{mIndex}] = cellfun(@(x, y) ttest(x, y), changeCellRowNum(ampS1{mIndex}), changeCellRowNum(rmsSponS1{mIndex}), "UniformOutput", false);
