...
ICAName = strcat(FIGPATH, "comp.mat");
...
if ~exist(ICAName, "file")
    ...
    print(FigWave(2), strcat(FIGPATH, "_IC_Rescutction_", protStr), "-djpeg", "-r200");
    print(FigTopoICA, strcat(FIGPATH, "_IC_Topo_", protStr), "-djpeg", "-r200");
    print(FigIC, strcat(FIGPATH, "_IC_Raw_", protStr), "-djpeg", "-r200");
    close(FigTopoICA);
    close(FigWave);
    close(FigIC);
...
for dIndex = devType
    ...
    % wave
    FigWave = plotRawWave(chMean{dIndex}, [], Window, stimStrs(dIndex), [8, 8]);
    FigWave_Whole = plotRawWave(chMean{dIndex}, [], Window, stimStrs(dIndex), [8, 8]);
    ...
    print(FigWave_Whole, strcat(FIGPATH, Protocol, "_", stimStrs(dIndex),  "_whole_Reg_Wave"), "-djpeg", "-r200");
    print(FigWave, strcat(FIGPATH, Protocol, "_", stimStrs(dIndex),  "_Reg_Wave"), "-djpeg", "-r200");
    print(FigTopo, strcat(FIGPATH, Protocol, "_", stimStrs(dIndex),  "_Topo_Reg"), "-djpeg", "-r200");
    ...
    print(FigPVal, strcat(FIGPATH, Protocol, "_", stimStrs(dIndex), "_pValue_Topo_Reg"), "-djpeg", "-r200");
    close(FigPVal);
