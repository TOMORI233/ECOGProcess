...
FIGPATH = strcat(ROOTPATH, "\Figure4\", stimStrs(stimSelect), "\");
mkdir(FIGPATH);
...
for mIndex = 1 : 5
    ...
    for dIndex = 1:length(devType)
        ...
        [temp, amp(dIndex).(protStr(mIndex)), rmsSpon(dIndex).(protStr(mIndex))] = cellfun(@(x) waveAmp_Norm(x, Window, quantWin, 1, sponWin), trialsECOG, 'UniformOutput', false);
...
%         [latency_mean, latency_se, latency_raw] = waveLatency_trough(trialsECOG, Window, quantWin, 10, fs); %
        [latency_mean, latency_se, latency_raw] = waveLatency_cumThreshold(trialsECOG, Window, quantWin, 0.3, fs); %
    ...
    if ~isempty(badCh{monkeyId})
        topo(badCh{monkeyId}) = 1;
    end
    FigTopo = plotTopo_Raw(topo, [8, 8]);
    colormap(FigTopo, "jet");
    scaleAxes(FigTopo, "c", [0.8 2]);
    set(FigTopo, "outerposition", [300, 100, 800, 670]);
    print(FigTopo, strcat(FIGPATH, DateStr, "_", Protocol, "_CRI_Topo_", protStr(mIndex), "_", stimStrs(stimSelect)), "-djpeg", "-r200");
    close(FigTopo);
...
plotLayout(FigWave, params.posIndex + 2 * (monkeyId - 1), 0.3);
 print(FigWave, strcat(FIGPATH, DateStr, "_", Protocol, "_Wave_", stimStrs(stimSelect)), "-djpeg", "-r200");
 close(FigWave);
...
for mIndex = 1 : length(MATPATH)
    ...
    OneArray = repmat({ones(length(temp{1}), 1)}, length(temp), 1);
    ...
    print(FigTopo(mIndex), strcat(FIGPATH, DateStr, "_", Protocols{mIndex}, "_pValue_Topo_", protStr(mIndex), "_", stimStrs(stimSelect)), "-djpeg", "-r200");
end
...
%         topo(~ismember(1 : 64, sigCh)) = quantWin(2);
        topo = quantWin(2) - topo;
        FigTopo = plotTopo_Raw(topo, [8, 8]);
        colormap(FigTopo, "jet");
        scaleAxes(FigTopo, "c", [100 230]);
        set(FigTopo, "outerposition", [300, 100, 800, 670]);
        print(FigTopo, strcat(FIGPATH, DateStr, "_", Protocol, "_latency_Topo_", protStr(mIndex), "_", stimStrs(stimSelect)), "-djpeg", "-r200");
        close(FigTopo);
    end
