function ME_MSTI_Update_Config
% load excel
...
orderIndex = cellfun(@(x) str2double(string(strsplit(x, "-"))), string(strsplit(mProtocol.orderIndex, ","))', "uni", false);

trialTypes = cellfun(@(x) strrep(MMNSequences{x(1)}(x(2)).Tag, "-", "_"), orderIndex, "uni", false);
