...
%% load parameters
ME256Params = ME256_ParseOffsetParams(protocolStr);
parseStruct(ME256Params)
...
tic
[trialAll, trialsECOG_Merge, trialsECOG_S1_Merge] =  selectCTLTrialsECOG(MATPATH, params.posIndex, ME256Params);
badCHs = [];
toc
...
chMeanS1 = cell(1, length(devType));
...
for dIndex = devType
    ...
    for ch = 1 : size(chMean{dIndex}, 1)
        ...
        cdrPlot(ch).(strcat(monkeyStr, "Wave"))(dIndex).Data(:, 2) = chMean{dIndex}(ch, :)';
        cdrPlot(ch).(strcat(monkeyStr, "WaveS1"))(dIndex).Data(:, 1) = t';
        cdrPlot(ch).(strcat(monkeyStr, "WaveS1"))(dIndex).Data(:, 2) = chMeanS1{dIndex}(ch, :)';
        cdrPlot(ch).(strcat(monkeyStr, "WaveFilted"))(:, 2 * dIndex - 1) = t';
        cdrPlot(ch).(strcat(monkeyStr, "WaveFilted"))(:, 2 * dIndex) = chMeanFilterd{dIndex}(ch, :)';
        cdrPlot(ch).(strcat(monkeyStr, "FFT"))(dIndex).Data(:, 1) = ff;
        cdrPlot(ch).(strcat(monkeyStr, "FFT"))(dIndex).Data(:, 2) = PMean{dIndex}(ch, :)';
    end
...
for gIndex = 1 : length(group_Index)
    % offset
    for dIndex = 1 : length(group_Index{gIndex})
        chPlot(dIndex).chMean = chMean{group_Index{gIndex}(dIndex)};
        chPlot(dIndex).color = colors(dIndex);
    end
    FigGroup = plotRawWave256Multi_SPR(chPlot, Window, group_Str(gIndex));
%     addLegend2Fig(FigGroup(4), stimStrs(group_Index{gIndex}));
    % onset
    for dIndex = 1 : length(group_Index{gIndex})
        chPlotS1(dIndex).chMean = chMeanS1{group_Index{gIndex}(dIndex)};
        chPlotS1(dIndex).color = colors(dIndex);
    end
    FigGroupS1 = plotRawWave256Multi_SPR(chPlotS1, Window, group_Str(gIndex));
...
% %% plot FFT
% for dIndex = devType
%     FigFFT = plotRawWave(PMean{dIndex}, [], [ff(1), ff(end)], strcat("FFT ", stimStrs(dIndex)));
