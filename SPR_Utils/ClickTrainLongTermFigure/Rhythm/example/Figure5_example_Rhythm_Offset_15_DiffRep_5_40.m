...
if monkeyId == 1
    MATPATH{1} = 'E:\ECoG\MAT Data\CC\ClickTrainLongTerm\TITS_Offset_15_DiffRep_5_40\cc20221111\cc20221111_AC.mat';
...
fhp = 0.1;
flp = 10;
stimStrs = ["Reg15_Rep0", "Reg15_Rep5", "Reg15_Rep10", "Reg15_Rep20", "Reg15_Rep40", "Irreg15_Rep1", "Irreg15_Rep5", "Irreg15_Rep10", "Irreg15_Rep20", "Irreg15_Rep40"];

protStr = "TITS_Offset_15_DiffRep_5_40";
...
for mIndex = 1 : length(MATPATH)
    ...
    FIGPATH = strcat(ROOTPATH, "Figure5_TITS_Offset_15_DiffRep_5_40\", dateStr, "\", AREANAME, "\");
    mkdir(FIGPATH);
    %% process
    tic
...
colors = ["#FF0000", "#FFA500","#00FF00" , "#0000FF", "#000000"];
for dIndex = 1 : 5
    [~, tIndex] = findWithinInterval(t, selWin -S1Duration(dev * 5 + dIndex) + changeTime(dev * 5 + dIndex));
    changeOffset(dIndex).chMean = chMean{1, dev * 5 + dIndex}(:, tIndex);
    changeOffset(dIndex).color = colors(dIndex);
end
...
% [~, tIndex] = findWithinInterval(t, selWin);
% changeOffset(3).chMean = chMean{1, dev3}(:, tIndex);
% changeOffset(3).color = "b";

% [~, tIndex] = findWithinInterval(t, selWin -S1Duration(dev2) + changeTime(dev2));
% changeOffset(3).chMean = chMean{1, dev2}(:, tIndex);
% changeOffset(3).color = "b";
FigCompare = plotRawWaveMulti_SPR(changeOffset, selWin);


%% check click train
soundPath = "E:\ratNeuroPixel\monkeySounds\2022-11-11_Offset_15_Rep\decoding";
...
resmpT = resample(soundT, length(t), length(soundT))  - changeTime(10);
