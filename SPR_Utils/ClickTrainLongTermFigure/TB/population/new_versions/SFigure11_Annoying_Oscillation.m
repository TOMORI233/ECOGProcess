...
MATPATH{1} = 'E:\ECoG\MAT Data\CC\ClickTrainLongTerm\TB\Add_on_Annoying_Oscillation\';
...
for mIndex = 1 : length(MATPATH)
    ...
    Protocol = temp(end - 1);
    FIGPATH = strcat(ROOTPATH, "\Pop_SFigure11_Osci_Control\",  SRIMethodStr(SRIMethod), "\", monkeyStr(mIndex), "\");
    mkdir(FIGPATH);
    %% process
    run("tb_loadData.m");
    run("CTLconfig.m");
