...
MATPATH{1} = 'E:\ECoG\MAT Data\CC\ClickTrainLongTerm\Basic_IrregVar\cc20220712\cc20220712_AC.mat';
MATPATH{2} = 'E:\ECoG\MAT Data\XX\ClickTrainLongTerm\Basic_IrregVar\xx20220712\xx20220712_AC.mat';
monkeyStr = ["CC", "XX"];
...
stimStrs = ["u_50", "u_100", "u_200", "u_400"];
...
for mIndex = 2 : length(MATPATH)
    ...
    DateStr = temp(end - 1);
    Protocol = temp(end - 2);
    FIGPATH = strcat(ROOTPATH, "\Figure5\", DateStr, "\", CRIMethodStr(CRIMethod), "\");
    mkdir(FIGPATH);

    if contains(DateStr, "20220702")
        cdrPlotIdx = [8, 6, 4, 2];
        colors = ["#000000", "#0000FF", "#FFA500", "#FF0000"];
    end
    %% process
