...
for blkN = 1 : length(blksActive)
    for posIndex = 1 : 2
        ...
        if ~exist(fullfile(SAVEPATH,strcat(posStr(posIndex), '_data.mat')),"file") % check if rawData exist
            ...
            mkdir(SAVEPATH);
            save(fullfile(SAVEPATH,strcat(posStr(posIndex), '_data.mat')), 'ECOGDataset', 'trialAll', '-mat');
        elseif  replotFigure || reprocess
            load(fullfile(SAVEPATH,strcat(posStr(posIndex), '_data.mat')));
        else
            continue
        end
        ...
        for dIndex = 1:length(devType)
            ...
            FigDev1(dIndex) = plotRawWave(chMean{sIndex}, chStd{sIndex}, window, ['stiTyme: ', num2str(pairStr{dIndex}), '(N=', num2str(length(trials)), ')']);
            drawnow;
            FigDev2(dIndex) = plotTFA(chMean{sIndex}, fs0, fs, window, ['stiTyme: ', num2str(pairStr{dIndex}), '(N=', num2str(length(trials)), ')']);
            drawnow;
        end
        ...
        for dIndex = 1 : length(devType)
            ...
            resultC{dIndex} = [resultC{dIndex}; result([trials.correct] == true)];
            resultW{dIndex} = [resultW{dIndex}; result([trials.correct] == false & [trials.interrupt] == false)];
            if length(resultW{dIndex})<0.1*length(trials) || length(resultC{dIndex})<0.1*length(trials)
                FigDM1(dIndex) = figure('Visible','off');
                FigDM2(dIndex) = figure('Visible','off');
                continue
            else
                chMeanStd{dIndex} = cell2mat(cellfun(@mean, changeCellRowNum(resultC{dIndex}), "UniformOutput", false));
                chMeanDev{dIndex} = cell2mat(cellfun(@mean, changeCellRowNum(resultW{dIndex}), "UniformOutput", false));
                FigDM1(dIndex) = plotRawWave(chMeanDev{dIndex} - chMeanStd{dIndex}, [], window, ['C', num2str(length(resultC{dIndex})), 'W', num2str(length(resultW{dIndex}))]);
                drawnow;
                FigDM2(dIndex) = plotTFACompare(chMeanDev{dIndex}, chMeanStd{dIndex}, fs0, fs, window, ['C', num2str(length(resultC{dIndex})), 'W', num2str(length(resultW{dIndex}))]);
                drawnow;
            end
        end
        ...
        % save MMN data
        save(fullfile(SAVEPATH,strcat(posStr(posIndex), '_MMNData.mat')), 'trialsC', 'trialsW', 'typeStr', 'pairStr', 'chMeanC', 'chMeanW', 'chMeanSTDC', 'chMeanSTDW', 'chMeanDEVC', 'chMeanDEVW', 'chStd', 'fs', 'fs0', 'window', 'stdType');
