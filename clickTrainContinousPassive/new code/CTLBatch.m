...
% rootPathMat = 'E:\ECoG\MAT Data\XX\ClickTrainLongTerm\successive_0o3_0o5\';
...
ROOTPATH = "E:\ECOG\Figures\ClickTrainLongTerm\successive_1_2\";
...
for fIndex = 1:length(temp)
    ...
    for mIndex = 1:length(MATFiles)
        ...
        if strcmp(splitName{end}, 'AC')
            CTLSingleFcn([rootPathMat, temp(fIndex).name, '\', MATFiles{mIndex}], ROOTPATH, 1);
        elseif strcmp(splitName{end}, 'PFC')
            CTLSingleFcn([rootPathMat, temp(fIndex).name, '\', MATFiles{mIndex}], ROOTPATH, 2);
        end
        close all;
    end
