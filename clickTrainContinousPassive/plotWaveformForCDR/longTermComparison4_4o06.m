...
for id = idIdx
    ...
    for pN = 1 : length(paradigmKeyword)
        ...
        for recordCode = 1 : length(matPath)
            ...
            dateStr = temp{5};
            savePath = fullfile("E:\ECoG\corelDraw\jpgHP0o1Hz", "freq band response comparison", dateStr, "4-4.06") ;
            ...
            tempRegWave2Rev = tempReg2Rev.chMean; % PFC 4-4.06 regular
            tempIrregWave2Rev = tempIrreg2Rev.chMean; % PFC 4-4.06 irregular
            ...
            if plotFigure
                ...
                for bN = 1 : size(bpRanges, 1)
                    ...
                    % AC
                    FigRawAC = plotRawWave(filtRegWave1Ord, [], window, strcat("AC Frequency Band ", bandStr(bN), " : [", num2str(bpRange(1)), " ", num2str(bpRange(2)), "]"), [8, 8], 1:64, "off");
                    setLine(FigRawAC, "LineWidth", 1, "LineWidth", 1.5);
                    lineSetting.color = "blue"; lineSetting.style = "-"; lineSetting.width = 1;
                    FigRawAC = plotRawWave2(FigRawAC, filtIrregWave1Ord, [], window, lineSetting, [8, 8]);
                    lines.X = -1 * LAuC_FilterRes(1).S1Duration;
                    lines.color = "black";
                    lines.width = 0.5;
                    ...
                    % PFC
                    FigRawPFC = plotRawWave(filtRegWave2Ord, [], window, strcat("PFC Frequency Band ", bandStr(bN), " : [", num2str(bpRange(1)), " ", num2str(bpRange(2)), "]"), [8, 8], 1:64, "off");
                    setLine(FigRawAC, "LineWidth", 1, "LineWidth", 1.5);
                    lineSetting.color = "blue"; lineSetting.style = "-"; lineSetting.width = 1;
                    FigRawPFC = plotRawWave2(FigRawPFC, filtIrregWave2Ord, [], window, lineSetting, [8, 8]);
                    lines.X = -1 * LPFC_FilterRes(1).S1Duration;
                    lines.color = "black";
                    lines.width = 0.5;
...
%                     yScaleS2 = [min(min(tempS2)), max(max(tempS2))];
                    ...
                    if bN == 1
                        FigEnvRegAC = plotRawWave(filtRegWave1Ord(:, zoomS2OnsetIdx), [], zoomWin, strcat("AC Frequency Band ", bandStr(bN), " : [", num2str(bpRange(1)), " ", num2str(bpRange(2)), "]"), [8, 8], 1:64, "off");
                        FigEnvRegAC = plotRawWave2(FigEnvRegAC, filtRegWave1Rev(:, zoomS1OnsetIdxRev), [], zoomWin, lineSetting, [8, 8]);
                        yl = scaleAxes(FigEnvRegAC, "y", yScaleS1(bN, :));
                    else
                        FigEnvRegAC = plotRawWave(envReg1Ord(:, zoomS2OnsetIdx), [], zoomWin, strcat("AC Frequency Band ", bandStr(bN), " : [", num2str(bpRange(1)), " ", num2str(bpRange(2)), "]"), [8, 8], 1:64, "off");
                        lineSetting.color = "red"; lineSetting.style = ":"; lineSetting.width = 1;
                        FigEnvRegAC = plotRawWave2(FigEnvRegAC, filtRegWave1Ord(:, zoomS2OnsetIdx), [], zoomWin, lineSetting, [8, 8]);
                        lineSetting.color = "blue"; lineSetting.style = "-"; lineSetting.width = 1.5;
                        FigEnvRegAC = plotRawWave2(FigEnvRegAC, envReg1Rev(:, zoomS1OnsetIdxRev), [], zoomWin, lineSetting, [8, 8]);
                        lineSetting.color = "blue"; lineSetting.style = ":"; lineSetting.width = 1;
                        FigEnvRegAC = plotRawWave2(FigEnvRegAC, filtRegWave1Rev(:, zoomS1OnsetIdxRev), [], zoomWin, lineSetting, [8, 8]);
                        yl = scaleAxes(FigEnvRegAC, "y", yScaleS1(bN, :));
                    end
                    ...
                    if bN == 1
                        FigEnvRegPFC = plotRawWave(filtRegWave2Ord(:, zoomS2OnsetIdx), [], zoomWin, strcat("AC Frequency Band ", bandStr(bN), " : [", num2str(bpRange(1)), " ", num2str(bpRange(2)), "]"), [8, 8], 1:64, "off");
                        FigEnvRegPFC = plotRawWave2(FigEnvRegPFC, filtRegWave2Rev(:, zoomS1OnsetIdxRev), [], zoomWin, lineSetting, [8, 8]);
                        yl = scaleAxes(FigEnvRegPFC, "y", yScaleS2(bN, :));
                    else
                        FigEnvRegPFC = plotRawWave(envReg2Ord(:, zoomS2OnsetIdx), [], zoomWin, strcat("PFC Frequency Band ", bandStr(bN), " : [", num2str(bpRange(1)), " ", num2str(bpRange(2)), "]"), [8, 8], 1:64, "off");
                        lineSetting.color = "red"; lineSetting.style = ":"; lineSetting.width = 1;
                        FigEnvRegPFC = plotRawWave2(FigEnvRegPFC, filtRegWave2Ord(:, zoomS2OnsetIdx), [], zoomWin, lineSetting, [8, 8]);
                        lineSetting.color = "blue"; lineSetting.style = "-"; lineSetting.width = 1.5;
                        FigEnvRegPFC = plotRawWave2(FigEnvRegPFC, envReg2Rev(:, zoomS1OnsetIdxRev), [], zoomWin, lineSetting, [8, 8]);
                        lineSetting.color = "blue"; lineSetting.style = ":"; lineSetting.width = 1;
                        FigEnvRegPFC = plotRawWave2(FigEnvRegPFC, filtRegWave2Rev(:, zoomS1OnsetIdxRev), [], zoomWin, lineSetting, [8, 8]);
                        yl = scaleAxes(FigEnvRegPFC, "y", yScaleS2(bN, :));
                    end
                    ...
                    ACS1 = filtRegWave1Rev(:, zoomS1OnsetIdxRev);  PFCS1 = filtRegWave2Rev(:, zoomS1OnsetIdxRev); 
                    ACS2 = filtRegWave1Ord(:, zoomS2OnsetIdx);  PFCS2 = filtRegWave2Ord(:, zoomS2OnsetIdx);
                    ...
                    % AC
                    FigXcorrRegAC = plotRawWave(r1, [], [lags1(1) lags1(end)], strcat("AC CCG Band ", bandStr(bN), " : [", num2str(bpRange(1)), " ", num2str(bpRange(2)), "]"), [8, 8], 1:64, "off");
                    ...
                    % PFC
                     FigXcorrRegPFC = plotRawWave(r2, [], [lags2(1) lags2(end)], strcat("PFC CCG Band ", bandStr(bN), " : [", num2str(bpRange(1)), " ", num2str(bpRange(2)), "]"), [8, 8], 1:64, "off");
