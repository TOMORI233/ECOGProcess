function BFLSingleFcnICA(MATPATH, ROOTPATH, posIndex, doICA)
narginchk(3, 4);
if nargin < 4
    doICA = false;
end
...
if doICA
    ...
    if ~exist(ICAName, "file")
        [ECOGDataset, FigICAWave, FigTopo]= toDoICA(ECOGDataset, trialAll, 500, ICAName);
        print(FigICAWave, strcat(MMNPATH, AREANAME, "_ICA_Wave_", DateStr), "-djpeg", "-r200");
        print(FigTopo, strcat(MMNPATH, AREANAME, "_ICA_Topo_",  DateStr), "-djpeg", "-r200");
    else
...
scaleAxes(FigPDiffWave, "y", [], yScale/3, "max");
